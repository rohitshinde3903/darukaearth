...
export default function ProjectDetailPage() {
  ...
  const fetchProject = async () => {
    try {
      const response = await fetch(`https://daruka.pythonanywhere.com/api/api/projects/${params.id}/`);
      if (response.ok) {
  ...
  const fetchSites = async () => {
    try {
      const response = await fetch(`https://daruka.pythonanywhere.com/api/api/sites/?project=${params.id}`);
      if (response.ok) {
  ...
  const handleDeleteSite = async (siteId: number) => {
    ...
    try {
      const response = await fetch(`https://daruka.pythonanywhere.com/api/api/sites/${siteId}/`, {
        method: 'DELETE',
      });
...
function CreateSiteModal({ projectId, onClose, onSuccess }: { projectId: string; onClose: () => void; onSuccess: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('https://daruka.pythonanywhere.com/api/api/sites/', {
        method: 'POST',
